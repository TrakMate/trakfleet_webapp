...
class _DeviceConfigInfoScreenState extends State<DeviceConfigInfoScreen> {
  ...
  final List<Map<String, String>> _defaultCommandButtons = [
    {'label': 'SHOW CONFIG', 'cmd': 'AT+GTCFG'},
    {'label': 'SHOW IOSTATUS', 'cmd': 'AT+GTIO'},
    {'label': 'START OTA', 'cmd': 'AT+GTOTA'},
    {'label': 'MOBILIZE', 'cmd': 'AT+GTMOB'},
    {'label': 'IMMOBILIZE', 'cmd': 'AT+GTIMM'},
  ];
  ...
  void _sendCommand(String command) {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      setState(() {
        _commandLogs[0]['receive'] = 'OK';
        _commandLogs[0]['type'] = 'Received';
      });
    });
  }
  ...
  Widget _buildCommandLogTable(bool isDark) {
    const int rowsPerPage = 10; // you can adjust this
    // Ensure pagination stays consistent with actual data size
    int totalPages =
        (_commandLogs.length / rowsPerPage)
            .ceil()
                              ...
                              _commandLogs.map((cmd) {
                                return DataRow(
                                  cells: [
                                    ...
                                    DataCell(Text(cmd['sent'] ?? '-')),
                                    DataCell(Text(cmd['receive'] ?? '-')),
                                    DataCell(Text(cmd['date'] ?? '-')),
                                    DataCell(Text(cmd['user'] ?? '-')),
                                  ],
                                );
                              }).toList(),
                        ),
